...
struct MEMORY_BLOCK next_fit_allocate(int request_size, struct MEMORY_BLOCK memory_map[MAPMAX], int *map_cnt, int process_id, int last_address) {
    ...
    for (int i = 0; i < *map_cnt; i++) {
        if (memory_map[i].process_id == 0 && memory_map[i].segment_size >= request_size) {
            int last_allocated_size = (memory_map[i].start_address == last_address) ? memory_map[i].segment_size : INT_MAX;
            if ((memory_map[i].start_address > last_address) && memory_map[i].segment_size >= last_allocated_size) {
                if (memory_map[i].segment_size == request_size) {
                    memory_map[i].process_id = process_id;
                    return memory_map[i];
                } else if (memory_map[i].segment_size > request_size) {
